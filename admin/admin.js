(() => {
  ...
  const f_precio_oferta = $("#f_precio_oferta");
  const f_stock = $("#f_stock");
  const f_destacado = $("#f_destacado");
  const f_oferta = $("#f_oferta");
  const f_nuevo = $("#f_nuevo");
  const f_mas_vendido = $("#f_mas_vendido");
  const f_stock_bajo = $("#f_stock_bajo");
  const f_img = $("#f_img");
  ...
  function render(){
    ...
    tbody.innerHTML = list.map(p => `
      <tr data-id="${escapeHtml(p.id)}">
        ...
        <td data-label="Precio">$${Math.round(p.precio).toLocaleString("es-AR")}${(p.oferta && p.precio_oferta && p.precio_oferta < p.precio) ? `<div class="muted" style="font-weight:800">Oferta: $${Math.round(p.precio_oferta).toLocaleString("es-AR")}</div>` : ""}</td>

        <td data-label="Oferta">${p.oferta ? '<span class="badge ok">Sí</span>' : '<span class="badge off">No</span>'}</td>

        <td data-label="Nuevo">${p.nuevo ? '<span class="badge ok">Sí</span>' : '<span class="badge off">No</span>'}</td>

        <td data-label="Más vendido">${p.mas_vendido ? '<span class="badge ok">Sí</span>' : '<span class="badge off">No</span>'}</td>
  ...
  function fillForm(p){
    ...
    f_precio_oferta.value = (p.precio_oferta && p.precio_oferta > 0) ? String(Math.round(p.precio_oferta)) : "";
    f_stock.checked = !!p.stock;
    f_destacado.checked = !!p.destacado;
    f_oferta.checked = !!p.oferta;
    f_nuevo.checked = !!p.nuevo;
    f_mas_vendido.checked = !!p.mas_vendido;
    f_stock_bajo.value = p.stock_bajo ? String(Math.round(p.stock_bajo)) : "0";
    f_img.value = p.img || "";
  }
  ...
  function readForm(){
    ...
    return normalize({
      ...
      precio,
      precio_oferta: Number(f_precio_oferta.value||0),
      stock: f_stock.checked,
      destacado: f_destacado.checked,
      oferta: f_oferta.checked,
      nuevo: f_nuevo.checked,
      mas_vendido: f_mas_vendido.checked,
      stock_bajo: Number(f_stock_bajo.value||0),
